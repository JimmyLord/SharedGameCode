...
void MenuButton::Draw(MyMatrix* matviewproj)
{
    ...
    if( pMesh && m_pBGMeshCamera )
    {
        ...
        int devw = devicerect.w;        //g_pGame->m_GameFullWidth
        int devh = devicerect.h;        //g_pGame->m_GameFullHeight
        ...
        if( m_pBGMeshLight )
        {
            ...
            lightpos.x = (((-ortholeft + lightpos.x) / devw) - 0.5f) * m_pBGMeshCamera->m_FrustumRightEdgeZ0*2;
            lightpos.y = (((-orthobttm + lightpos.y) / devh) - 0.5f) * m_pBGMeshCamera->m_FrustumTopEdgeZ0*2;
